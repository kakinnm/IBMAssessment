...
// Add column to support mark obsoleted product

app.get('/product', function (req, res) {
  db.addColumn('obsoleted product', { type: 'string' },  (err, rows) => {
    if (err) {
...
app.get('/product', function (req, res) {
  db.all("SELECT * FROM product WHERE not exists (Select obsoleted FROM obsoleted product)", (err, rows) => {
    if (err) {
      res.status(400).json({ "error": err.message });
      return;
    }
